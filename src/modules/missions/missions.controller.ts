...
export class MissionsController {
  ...
  @Auth()
  @Post("action/:id/start")
  @ApiOperation({ summary: "Generate X mission URL" })
  actionStart(@User() user: UsersDocument, @Param("id") id: string) {
    return this.missionsService.actionStart(user, id);
  }

  @Auth()
  @Post("action/:id")
  @ApiOperation({ summary: "User performs task" })
  action(@User() user: UsersDocument, @Param("id") id: string, @Body() data: MissionXVerifyDto) {
    return this.missionsService.action(user, id, data);
  }
